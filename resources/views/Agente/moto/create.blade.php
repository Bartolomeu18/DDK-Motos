...
<div class="w-full max-w-lg bg-white rounded-lg shadow-md p-6 m-10">
    <div class="text-center mb-6">
        <img src="{{ asset('img/ddk-logo-rbg.png') }}" alt="Moto" class="w-16 h-16 mx-auto mb-2">
    ...
    <form method="POST" action="{{ route('motos.store') }}">
            ...
            <select name="motoqueiro_id" id="motoqueiro_id" required
                ...
                @foreach ($motoqueiros as $motoqueiro)
                    <option value="{{ $motoqueiro->id }}">
                        {{ $motoqueiro->name }} ({{ $motoqueiro->email }})
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="observacoes" id="observacoes" rows="3"
                      class="mt-1 block w-full px-4 py-2 border rounded-md focus:ring-red-500 focus:border-red-500">{{ old('observacoes') }}</textarea>
        </div>
